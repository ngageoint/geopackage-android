...
public class EqualAreaAzimuthalProjection extends AzimuthalProjection
{
	...
	public Object clone()
	{
		...
		if (apa != null) p.apa = (double[]) apa.clone();
		return p;
	}
		...
		if (spherical)
		{
			...
			switch (mode)
			{
				case EQUATOR:
					...
					break;
				case OBLIQUE:
					lp.y = Math.abs(rh) <= EPS10 ? projectionLatitude : Math.asin(cosz * sinphi0 + y * sinz * cosphi0 / rh);
					x *= sinz * cosphi0;
			...
			lp.x = (y == 0. && (mode == EQUATOR || mode == OBLIQUE)) ? 0. : Math.atan2(x, y);
		}
		else
		{
	...
	// FIXME: requires awt
	/*
	 * public Shape getBoundingShape() { double r = 1.414 * a; return new
	 * Ellipse2D.Double( -r, -r, 2*r, 2*r ); }
	 */
