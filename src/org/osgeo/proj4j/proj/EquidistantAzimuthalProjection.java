...
public class EquidistantAzimuthalProjection extends AzimuthalProjection
{
	...
	public Object clone()
	{
		...
		if (en != null) p.en = (double[]) en.clone();
		return p;
	}
		...
		if (spherical)
		{
			...
			switch (mode)
			{
				...
				case OBLIQUE:
					...
					if (Math.abs(Math.abs(xy.y) - 1.) < TOL) if (xy.y < 0.) throw new ProjectionException();
					else xy.x = xy.y = 0.;
					else
					{
						xy.y = Math.acos(xy.y);
				...
				case NORTH_POLE:
					...
					coslam = -coslam;
				case SOUTH_POLE:
					if (Math.abs(phi - ProjectionMath.HALFPI) < EPS10) throw new ProjectionException();
		...
		else
		{
			...
			switch (mode)
			{
				case NORTH_POLE:
					coslam = -coslam;
				case SOUTH_POLE:
					xy.x = (rho = Math.abs(Mp - ProjectionMath.mlfn(phi, sinphi, cosphi, en))) * Math.sin(lam);
					xy.y = rho * coslam;
					break;
				case EQUATOR:
				case OBLIQUE:
					...
					cA = Math.cos(Az);
					sA = Math.sin(Az);
					s = ProjectionMath.asin(Math.abs(sA) < TOL ? (cosphi0 * st - sinphi0 * coslam * ct) / cA : Math.sin(lam) * ct / sA);
					H = He * cA;
					H2 = H * H;
	...
	public ProjCoordinate projectInverse(double x, double y, ProjCoordinate lp)
	{
		if (spherical)
		{
			...
			if (mode == OBLIQUE || mode == EQUATOR)
			{
				...
				{
					lp.y = ProjectionMath.asin(cosc * sinphi0 + y * sinc * cosphi0 / c_rh);
		...
		else
		{
			...
			if (mode == OBLIQUE || mode == EQUATOR)
			{
				...
				else if (Math.abs(t - ProjectionMath.HALFPI) < 0.) lp.y = ProjectionMath.HALFPI;
	...
	// FIXME: requires awt
	/*
	 * public Shape getBoundingShape() { double r = ProjectionMath.HALFPI * a;
